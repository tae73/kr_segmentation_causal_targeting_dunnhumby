...
다중공선성 문제를 해결하기 위해, 높은 상관관계를 가진 쌍 (r ≥ 0.7)을 제거하여 Feature 셋을 33개에서 **19개로 축소**하였다.

**다중공선성 처리 세부사항:**

| 제거 기준 | 제거된 Feature 예시 | 유지된 Feature |
|-----------|---------------------|----------------|
| 완전 상관 (r = 1.0) | frequency_per_month | frequency_per_week |
| 높은 상관 (r ≥ 0.9) | monetary_actual | monetary_sales |
| 중복 정보 (r ≥ 0.7) | active_last_12w | active_last_4w |
...
**참고:** VIF 분석보다 상관 기반 제거를 선택한 이유는 (1) NMF의 비음수 제약과 호환성 유지, (2) 해석 용이성 우선. 대안으로 Elastic Net 정규화를 통한 자동 변수 선택도 가능하나, 본 연구에서는 명시적 제거를 통해 재현성을 확보하였다.
...
| 기준 | NMF | PCA |
|------|-----|-----|
| **해석력** | Parts-based 분해 → 직관적 Factor 해석 | 직교 축 → 해석 어려움 |
| **비음수 제약** | 자연스러운 비음수 Loading | 음수 Loading 허용 |
| **비즈니스 적합성** | "고객 A = 0.3×충성 + 0.5×신선" 해석 가능 | "고객 A = PC1 - 0.2×PC2" 해석 불명확 |
| **마케팅 협업** | 부분 합산 해석으로 비기술팀과 소통 용이 | 기술적 설명 필요 |
| **선행 연구** | 리테일 세분화에서 광범위 사용 (Lee & Seung, 1999) | 일반적 차원 축소 |

**실험적 검증:** 동일 k에서 NMF Factor가 PCA Component보다 카테고리/행동 측면에서 더 명확한 클러스터링을 생성함을 확인하였다.
